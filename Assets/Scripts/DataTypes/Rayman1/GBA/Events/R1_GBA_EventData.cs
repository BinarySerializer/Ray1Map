        ...
        public override void SerializeImpl(SerializerObject s)
        {
            ...
            if(ETAPointer != null) 
            {
                do
                {
                    ...
                    // Serialize subetats
                    var prevETA = ETA;
                    ETA = new R1_EventState[EtatPointers.Length][];
                    for (int j = 0; j < EtatPointers.Length; j++)
                    {
                        ...
                        s.DoAt(EtatPointers[j], () => ETA[j] = s.SerializeObjectArray<R1_EventState>(ETA[j], count, name: $"{nameof(ETA)}[{j}]"));

                        if (ETA[j] == null)
                            ETA[j] = new R1_EventState[count];
                    }
