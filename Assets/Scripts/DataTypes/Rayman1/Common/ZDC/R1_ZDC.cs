...
namespace R1Engine
{
    public class R1_ZDC
    {
        private static byte[] R1PC_Type_ZDC => new byte[]
        {
        ...
        private static byte[] R1PC_ZDCTable => new byte[]
        {
        ...
        public void Serialize(SerializerObject s)
        {
            ...
            switch (s.GameSettings.EngineVersion)
            {
                ...
                case EngineVersion.R1_DSi:
                    ...
                    typeLength = 272;
                    tableLength = 200;
                    fromMemoryStreams(() => R1PC_Type_ZDC, () => R1PC_ZDCTable);
                ...
                case EngineVersion.R1_PC_Kit:
                    typeLength = 281;
                    tableLength = 200;
                    fromMemoryStreams(() => KitPC_Type_ZDC, () => KitPC_ZDCTable);
                    break;
                ...
                default:
                    Debug.Log($"No ZDC data implemented for current engine version");
                    return;
            }

            // Helper method for getting tables from memory streams
            void fromMemoryStreams(Func<byte[]> getTypeZDC, Func<byte[]> getZDCTable)
            {
                var typeKey = $"TypeZDC{s.GameSettings.EngineVersion}";

                if (!s.Context.FileExists(typeKey))
                {
                    var typeStream = new MemoryStream(getTypeZDC());
                    s.Context.AddFile(new StreamFile(typeKey, typeStream, s.Context));
                }

                typeOffset = s.Context.GetFile(typeKey).StartPointer;

                var tableKey = $"ZDCTable{s.GameSettings.EngineVersion}";

                if (!s.Context.FileExists(tableKey))
                {
                    var tableStream = new MemoryStream(getZDCTable());
