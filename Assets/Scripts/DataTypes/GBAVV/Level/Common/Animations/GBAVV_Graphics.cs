    ...
    public class GBAVV_Graphics : R1Serializable
    {
        ...
        public override void SerializeImpl(SerializerObject s)
        {
            if (!s.GameSettings.GBAVV_IsFusion)
            {
                ...
                AnimSets = s.DoAt(AnimSetsPointer, () => s.SerializeObjectArray<GBAVV_AnimSet>(AnimSets, AnimSetsCount, name: nameof(AnimSets)));
                ...
                Palettes = s.DoAt(PalettesPointer, () => s.SerializeObjectArray<GBAVV_ObjPalette>(Palettes, PalettesCount, name: nameof(Palettes)));
            }
            else
            {
                ...
                for (int i = 0; i < pointers.Length; i++)
                    AnimSets[i] = s.DoAt(new Pointer(pointers[i], Offset.file), () => s.SerializeObject<GBAVV_AnimSet>(AnimSets[i], name: $"{nameof(AnimSets)}[{i}]"));
            }
        }
    }
}