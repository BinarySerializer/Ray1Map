...
namespace R1Engine
{
    public class GBACrash_WorldMap_MapLayer : R1Serializable
    {
        ...
        public GBACrash_Isometric_MapLayer TileMap { get; set; }
        ...
        public override void SerializeImpl(SerializerObject s)
        {
            ...
            TileMap = s.DoAt(TileMapPointer, () => s.SerializeObject<GBACrash_Isometric_MapLayer>(TileMap, x => x.IsWorldMap = true, name: nameof(TileMap)));
            var mapTilesLength = TileMap.TileMapRows.SelectMany(x => x.Commands).Select(x => x.Params?.Max() ?? x.Param).Max() + 1;
            MapTiles = s.DoAt(MapTilesPointer, () => s.SerializeObjectArray<MapTile>(MapTiles, mapTilesLength * 4, x => x.GBACrash_IsWorldMap8bpp = Is8bpp, name: nameof(MapTiles)));
        }
    }
}