...
namespace Ray1Map.GBAIsometric
{
    public class GBAIsometric_Spyro_MapLayer : BinarySerializable
    {
        ...
        public GBAIsometric_IceDragon_ResourceRef TileAssembleIndex { get; set; }
        ...
        {
            MapIndex = s.SerializeObject<GBAIsometric_IceDragon_ResourceRef>(MapIndex, x => x.Pre_HasPadding = true, name: nameof(MapIndex));
            TileAssembleIndex = s.SerializeObject<GBAIsometric_IceDragon_ResourceRef>(TileAssembleIndex, x => x.Pre_HasPadding = true, name: nameof(TileAssembleIndex));
            TilesetIndex = s.SerializeObject<GBAIsometric_IceDragon_ResourceRef>(TilesetIndex, x => x.Pre_HasPadding = true, name: nameof(TilesetIndex));
