...
{
    public class GBAIsometric_Dragon_ROM : GBAIsometric_IceDragon_BaseROM
    {
        ...
        public GBAIsometric_IceDragon_ResourceRef[] LevelObjectIndices_Spyro2_Agent9 { get; set; }
        ...
        public override void SerializeImpl(SerializerObject s)
        {
            ...
            {
                x.Length = settings.LevelDataCount;
            ...
            if (s.GetR1Settings().EngineVersion == EngineVersion.GBAIsometric_Spyro2)
            {
                ...
                LevelObjectIndices_Spyro2_Agent9 = objIndices.Select(x => GBAIsometric_IceDragon_ResourceRef.FromIndex(s, x)).ToArray();
            ...
            LevelMaps = s.DoAt(pointerTable.TryGetItem(Spyro_DefinedPointer.LevelMaps), () => s.SerializeObjectArray<GBAIsometric_Spyro_LevelMap>(LevelMaps, settings.LevelMapsCount, name: nameof(LevelMaps)));

            // Serialize object data
            ObjectTypes = s.DoAt(pointerTable.TryGetItem(Spyro_DefinedPointer.ObjectTypes), () => s.SerializeObjectArray<GBAIsometric_ObjectType>(ObjectTypes, settings.ObjectTypesCount, name: nameof(ObjectTypes)));
            AnimSets = s.DoAt(pointerTable.TryGetItem(Spyro_DefinedPointer.AnimSets), () => s.SerializeObjectArray<GBAIsometric_Spyro_AnimSet>(AnimSets, settings.AnimSetsCount, name: nameof(AnimSets)));

            // Serialize portraits
            PortraitSprites = s.DoAt(pointerTable.TryGetItem(Spyro_DefinedPointer.PortraitSprites), () => s.SerializeObjectArray<GBAIsometric_Spyro_PortraitSprite>(PortraitSprites, settings.PortraitsCount, name: nameof(PortraitSprites)));

            // Serialize dialog entries
            DialogEntries = s.DoAt(pointerTable.TryGetItem(Spyro_DefinedPointer.DialogEntries), () => s.SerializeObjectArray<GBAIsometric_Spyro_Dialog>(DialogEntries, settings.DialogCount, name: nameof(DialogEntries)));

            // Serialize level properties
            GemCounts = s.DoAt(pointerTable.TryGetItem(Spyro_DefinedPointer.GemCounts), () => s.SerializeArray<ushort>(GemCounts, settings.PrimaryLevelCount, name: nameof(GemCounts)));
